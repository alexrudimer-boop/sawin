...
Statement used: For the Artin embedding P_n -> Aut(F_n), every finite-index subgroup of P_n contains a principal congruence kernel ker(P_n -> Aut(F_n/K)) for some characteristic finite-index subgroup K <= F_n.

Status: Used as external literature input.  McReynolds records Thurston's proof for pure braid groups.  The full B_n fixed-arity theorem below adds an elementary congruence quotient detecting the strand permutation.
...
Detector: Y is the conjugation rack of G=F_n/K, where K=K_0 cap K_1 combines the mod-2 permutation quotient with the characteristic finite-index subgroup supplied by pure braid CSP.
