...
The strand-carrier ledger can also be canonicalized.  The helper

```text
universal_k_canonical_strand_carrier_rows(...)
```

forms the least finite equivalence relation generated by the swapped
strand-carrier equations across all local rows.  Its audit

```text
universal_k_canonical_strand_carrier_soundness_audit(...)
```

checks whether the canonical relation is still fibrewise injective.  If it
is, then
...
proves the corresponding all-`n` residual-faithfulness subcase.  The automatic
residual-faithfulness selector now tries this canonical strand-carrier theorem
before the canonical coordinate-label theorem, and the top-level post-linear
audit exposes the explicit flag
