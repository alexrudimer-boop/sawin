...
endpoint potential telescoping identity
terminal readout generated by final evaluated longitudes
...
The decisive replacement is the fixed-assignment detector-lift telescoping
gate.  A valid audit must cover exactly the interval-derived `D_Gamma` row
keys and must supply a positive finite number of detector tracks chosen
before the braid word is read.  Their initial assignments are derived from
the initial interval data, each track follows the evaluated Artin detector
recurrence, and the endpoint potential starts at the identity and updates by
`P_{t+1}=P_t h_t` for the emitted label of the current signed row.  The
terminal potential must then be proved to lie in the subgroup generated by
the terminal evaluated detector longitudes.  Only this telescope converts
the finite signed row table into the all-`n` conclusion
`endpoint_E(beta) in V_beta(H_E)`.
...
signed_endpoint_generator_endpoint_potential_telescoping_verified
signed_endpoint_generator_terminal_readout_longitudes_verified
signed_endpoint_generator_telescoping_initial_readout_normalized
