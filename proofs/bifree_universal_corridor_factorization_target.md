...
commutator endpoint.  The ordinary abelian subgroup is now exact by
`proofs/abelian_longitude_image_criterion.md`: for finite abelian `A`,
`V_beta(A)` is generated by `a^{m_ij}` over the recursive-longitude exponent
