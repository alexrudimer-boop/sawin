...
16 non-permutation size-three rows certified
13 direct full-product stabilizer rows
3 subproduct-trivial rows
full_product_kernel_image_size distribution {1: 16}
full_product_quotient_size distribution {1: 16}
```

The direct rows cover the identity row and all q=2 one-component rows.  The
subproduct partial covers one `(3,4)` row by its q=3 subproduct and two
`(2,3,5)` rows by their `(2,3)` subproducts.  This is partial arity-8
evidence: 39 of the 55 arity-8 rows remain uncertified by this probe, and the
