...
The general finite abelian version is recorded separately in
`proofs/abelian_longitude_image_criterion.md`: for any finite abelian group
`A`, the subgroup `V_beta(A)` is generated by `a^{m_ij}` over all elements
