...
As in the general local closure audit, the product-label closure now records
first-derivation rows for each nontrivial relation edge.  The row source is
`seed`, `label_forward`, or `label_inverse`, and the source pair records which
already-related fibre pair was transported by the product label.  Thus a
universal product-label closure can be checked as a concrete finite
permutation-groupoid derivation, while a failure identifies the exact
generated edge graph that remains imprimitive.
