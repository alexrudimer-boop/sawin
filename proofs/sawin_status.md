...
In the first case the obstruction is central monolithic depth; in the second
case `H/C_H(M)` is a faithful irreducible linear action on `M`, and the moved
value is generated by conjugation differences.  Thus a negative proof must now
produce an unbounded central-depth tail, an unbounded irreducible linear-module
tail, or the nonabelian simple-product tail.  A positive proof may close the
global point-pushing route by ruling out all three for finite YBE action
images.
