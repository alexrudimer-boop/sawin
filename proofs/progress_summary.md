...
The note `proofs/green_section_transport_rigidity_gate.md` and generated
audit `proofs/green_section_transport_rigidity_gate_audit.md` isolate the
next Green/corridor rigidity question.  The comparison of hidden section
branches must be made inside the actual deterministic completed-context
category generated by supported YBE rows:

```text
R_q(aw)=a^q R_{q^a}(w).
```

Raw Rees coordinates, Green `R`-class normalization, and ambient
Schutzenberger permutations are not enough unless they produce an objectwise
transport
...
branches, a cyclic p-primary branch quotient, and unequal units
...
is checked by a finite deletion-decorated labelled residual graph.  Its
based loop-label subgroup `Lambda_{I,s,N,n,c}` is generated by
spanning-tree cycle labels, and the finite endpoint witness subgroup is
...
The native tetrahedral-module audit
`proofs/affine_f2_q3_tetrahedral_module_audit.md` sharpens that target.  It
identifies representative `24` as the Alexander rack on `F_2^2` with
`a*b=T b+(I+T)a`, `T` row masks `(2,3)`, and local braid row masks
`(11,13,1,2)`.  It also extracts from the affine `F_2^3` survivor an explicit
`2n-2` dimensional affine fibre module generated by the local offsets.  Native
group generation shows that, for arities `2`--`5`, the rack24 image, full `X`
image, induced fibre affine image, rack--`X` joint image, rack--fibre joint
image, and fibre--`X` joint image all have the same order:
