...
`closed_detector_group_orders`, and `closed_detector_gaps` now expose the
actual finite local detector groups for `product_finite_g_branch`,
`known_total_branch`, and `locally_nondegenerate_branch` verdicts whenever
the current proof ledger supplies them.  This is the construction-level handoff
needed before passing local factors to `assemble_congruence_chain_rack`.
