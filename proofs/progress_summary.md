...
identity-base cyclic, and known-total product rows.  Fibre-size-two affine
product rows remain honest delegated closures: the summary records a
certificate gap pointing to the affine `F_2` branch note rather than silently
assigning a local cyclic detector that has not been proved in this wrapper.
...
closed product detector certificates and any delegated affine gaps.  The
remaining corridor fork is reached only after these explicit product/known
detector records fail to apply.
