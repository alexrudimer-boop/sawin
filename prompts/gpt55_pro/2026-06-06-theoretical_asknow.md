...
56. Parabolic-disjoint chief obstruction.

The repo now records the parabolic-disjoint refinement in

  proofs/parabolic_disjoint_chief_obstruction.md

You may use it as established context.

Use the strengthened detector

  Q_{m,s}=(P_m x Y_mu)^0 x T_2 x B_s(X).

For the last-strand image, write
...
For T subset {1,...,n-1}, define

  F_T=<x_i:i in T>,
  H_T=Phi_n(F_T).

Then for every m,s,N there exists n>N and a chief factor A/B of Gamma_n such
that

  B < A <= K_n cap C_n,

the X-projection of A is nontrivial and generates the first-fold monolith
mu, and for every bounded subset |T|<=s,
...
- Brunnian finite-image obstruction gives K_n cap C_n != 1 in unbounded
  arities.
- Choose A/B inside K_n cap C_n.
- Since A<=K_n and Q_{m,s} contains Y_mu^0, p_X(A) acts trivially on
  (X/mu)^n.
- Since K_n cap L_n=1, p_X(A) is nontrivial.
- Therefore the coordinate mismatches generated by p_X(A) are nontrivial,
  lie inside mu, and hence equal mu because mu is the global monolith.
- Since Q_{m,s} contains B_s(X), bounded-support annihilation gives
  K_n cap H_T=1 for every |T|<=s.
- If a in A cap H_T B, write a=hb.  Since A,B<=K_n, h=ab^{-1} lies in
  K_n cap H_T=1, so a in B.

Consequences:

- Pairwise branch: if pairwise commutators cover A/B, they do so only after
  conjugation by unboundedly many other meridians.  It is a global conjugation
  ghost, not a bounded-strand obstruction.
- Central abelian branch: if A/B is a central elementary p-factor, every
  bounded parabolic restriction still vanishes.  It is a high-Brunnian
  module class, not a bounded-support p-module.

The next exact A target is:

  rule out parabolic-disjoint monolith-generating chief factors for
  first-fold atoms.

The next exact B target is:

  construct one fixed finite X whose first-fold monolith is generated
  cofinally by parabolic-disjoint all-meridian chief factors.

57. Deletion-null monolith chief obstruction.

The repo now records the deletion-null strengthening in

  proofs/deletion_null_monolith_chief_obstruction.md

You may use it as established context.

With the strengthened detector
...
For every rack prefix P_m, support cutoff s, and arity cutoff N, there is
n>N and a chief factor A/B of Gamma_n with

  B < A <= K_n cap C_n,

whose X-projection generates the first-fold monolith mu, and which remains
parabolic-disjoint:

  A cap H_T B = B        for every |T|<=s.

Here F_T=<x_i:i in T> and H_T=Phi_n(F_T).

Moreover, every nontrivial element a in A has an actual Brunnian
Q_{m,s}-invisible representative:

  exists beta_a in Brun_n cap ker rho^{Q_{m,s}}_n
  with Phi_n(beta_a)=a.

Therefore every proper deletion of beta_a is literally trivial in the braid
group:

  d_I(beta_a)=1        for every proper I subsetneq {1,...,n}.

For every 1!=a in A, the X-projection p_X(a) is nontrivial because
K_n cap L_n=1, acts inside mu-fibres because A<=K_n and Q_{m,s} contains
Y_mu^0, and its coordinate mismatches generate mu because mu is the global
monolith.

Thus the obstruction is not merely large support, pairwise normal-closure
noise, or finite-image same-image deletion coherence.  It is an exact
deletion-null all-meridian chief factor: every nontrivial element lifts to an
ordinary Brunnian braid in the original braid group.
...
61. Current exact unresolved gap.

The repo now records the current failed endpoint in

  proofs/actual_mismatch_probe_visibility_gap.md

You may use it as established context.

The tempting A proof tries to use

  mu <= F_1(X)

to force probe visibility.  The desired implication is:
...
This implication is not proved.

The obstruction chief factor only says that the actual coordinate mismatches
of p_X(A) generate the monolith mu as a braided congruence.  It does not say
that any actual moved pair is itself an immediate first-fold degeneracy pair.
The first-fold pair may appear only after closing under R_X, R_X^{-1}, and
transitivity.
...
Also, repeated derivatives do not currently contradict bounded-support
annihilation: their support may grow with arity, while bounded-support
detectors only kill fixed-support representatives.

The tempting B proof still lacks one fixed finite X whose first-fold monolith
supports cofinal deletion-null Brunnian chief factors

  K_n cap C_n

against every finite rack prefix.  The known powered alternating-group
construction varies the target with the detector and therefore does not prove
B.

So the exact current status is:

  A positive resolution must rule out first-fold monoliths generated cofinally
  by actual deletion-null detector-invisible Brunnian chief factors, or prove
  enough probe visibility/derivative collapse to kill the two branches.

  A negative resolution must construct one fixed finite X where such cofinal
  monolith-generating chief factors occur against every finite rack prefix.

62. Probe-central motion congruence.

The repo now records the finite motion-congruence repair in

  proofs/probe_central_motion_congruence.md

You may use it as established context.

For the conjugated-probe core branch, define

  H_n=K_n cap C_n cap CPC^Gamma_n.

Define Pi_{Q,n} to be the braided congruence on X generated by all coordinate
color changes caused by elements of H_n:

  Pi_{Q,n}
    =
  < (a,b) :
      exists h in H_n, exists x in X^n, exists j,
      x_j=a and (p_X(h)x)_j=b and a != b
  >_br.

This is finite and computable from Gamma_n.

If the conjugated-derivative-visible branch fails cofinally, then for every
rack prefix, support cutoff, and arity cutoff, there exists n beyond the
cutoff with

  Pi_{Q,n}=mu.

Proof sketch: H_n <= K_n, and Q contains Y_mu^0 with Y_mu dominating X/mu, so
all H_n-coordinate mismatches lie inside mu; hence Pi_{Q,n} <= mu.  The
deletion-null chief factor in the core branch lies inside H_n, and
K_n cap L_n=1 makes its X-projection nontrivial; hence Pi_{Q,n} is nontrivial.
Since mu is the monolith, Pi_{Q,n}=mu.

This fixes the previous false extraction attempt:

  actual moved pairs need not be first-fold pairs,

but in the probe-central core branch their H_n-motion congruence regenerates
the monolith.

The current exact obstruction split is:

- conjugated-derivative tower:

    nontrivial conjugated derivatives persist cofinally;

- probe-central monolith regeneration:

    Pi_{Q,n}=mu occurs in unbounded arity.

The next exact A target is:

  prove no infinite conjugated-derivative tower can persist, and prove
  Pi_{Q,n} != mu eventually for some finite detector Q.

The next exact B target is:

  construct one fixed finite X whose first-fold monolith supports either
  cofinal conjugated derivatives or unbounded Pi_{Q,n}=mu against every finite
  rack prefix.

63. Abelian contextual separability test.

The repo now records the abelian contextual separability layer in

  proofs/abelian_contextual_separability_test.md

You may use it as established context.

The contextual Wirtinger group

  G_X=<g_p | same-strand equalities,
              g_r=g_p g_q g_p^{-1} for forced p*q=r>

has abelianization generated by contextual classes [p], with finite linear
relations

  [p]=[p']       for same-strand equalities,
  [r]=[q]        for forced compatible products p*q=r.

If

  [g_p] != [g_{p'}] in G_X^ab,

then p and p' are separated by a finite abelian contextual augmented quotient.

If p,p' lie in the same contextual action component, let tau label a path from
p to p' and let Lambda_p be the loop subgroup at p.  If

  [tau] notin im(Lambda_p -> G_X^ab),

then p and p' are also separated by a finite abelian contextual augmented
quotient.

Thus any contextual transition that survives every finite contextual
augmented quotient must pass both abelian tests:
...
So any remaining contextual separability failure is genuinely nonabelian:

  tau in closure_prof(Lambda_p) \ Lambda_p

while tau is already abelianly indistinguishable from a loop.

The next positive target is:

  prove subgroup separability for the YBE-origin loop subgroups after this
  abelian obstruction vanishes.

The next negative target is:

  construct a finite X whose contextual Wirtinger group has a genuinely
  nonabelian nonseparable loop subgroup, with the nonseparable transporter
...
66. Powered-meridian no-freezing theorem.

The repo now records the fixed-target no-freezing theorem in

  proofs/powered_meridian_no_freezing.md

You may use it as established context.

Let X be a fixed finite bijective YBE solution and put
...
Reason: for every d there is a finite rack Y_d=Z/dZ with operation

  a*b=b+1.

Its rack braid map satisfies

  R(a,b)=(b+1,a),
  R^2(a,b)=(a+1,b+1),

so ord(rho^{Y_d}_2(sigma_1^2))=d.  The rack prefix eventually contains
Y_{d_X}, so e_m is eventually divisible by d_X.

Every pure meridian A_{ij} is conjugate to sigma_1^2, so for large m,

  rho^X_n(A_{ij}^{e_m})=1

for every i,j.  Hence the entire normal subgroup generated by the powered
meridians A_{ij}^{e_m} acts trivially on X^n.

Therefore no fixed-target counterexample can be built from Brunnian words in
the powered meridians

  W(A_{1,n}^{e_m},A_{2,n}^{e_m},...,A_{n-1,n}^{e_m}),

including the standard powered commutators.

The known detector-survival construction works only because the target varies
with the detector, e.g. X_Q=Conj(A_l) with l chosen not to divide the
detector's pure order.

For one fixed finite X, that strategy collapses once the rack prefix absorbs
d_X.  A genuine negative construction must use Brunnian witnesses whose
detector-invisibility is a global finite-rack law, not a consequence of each
meridian becoming individually trivial.

So any fixed-target counterexample sequence must have, for large m,
