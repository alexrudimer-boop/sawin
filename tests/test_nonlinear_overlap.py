...
def two_color_coordinate_identity_swap_interval():
    colors = ("a", "b")
...
class OneNoTriangularRawKRefinement:
    status = "triangular_recovery_kink_completion_deficit"
    active_missing_left_latin_row_defects = ((("*", "*"), "no_left_triangular_row"),)
    active_missing_right_latin_row_defects = ()


def residual_trivial_identity_observer_cases():
    return (
        (
            "singleton_fibre",
...
class RecoveryAndMixedRawKRefinement:
    """Fixture with one U-routed recovery row and one M-routed mixed row."""

    def __init__(self):
        self._base = constant_map_kernel_system_k_refinement()

    def __getattr__(self, name):
        return getattr(self._base, name)

    @property
    def active_missing_left_latin_row_defects(self):
        return (
    ...
    @property
    def active_missing_right_latin_row_defects(self):
        return ()


class RecoveryContinuationMixedRawKRefinement:
    """Fixture with U, C, and M routed endpoint rows active together."""
    ...
    @property
    def active_missing_right_latin_row_defects(self):
        return ((("*", "*"), "no_right_triangular_row"),)
...
class NonlinearOverlapObstructionAuditTests(unittest.TestCase):
    ...
    def test_post_linear_profile_routing_rejects_inexact_section_rank_row(self):
        interval = one_color_right_triangular_nonlatin_interval()
        profile = missing_triangular_row_profile_audit(interval)
        forged_section = replace(
            profile.rows[0].section_profiles[0],
            rank=profile.rows[0].section_profiles[0].rank + 1,
        )
        forged_profile = MissingTriangularRowProfileAudit(
            rows=(
                replace(
                    profile.rows[0],
                    section_profiles=(
                        forged_section,
                        *profile.rows[0].section_profiles[1:],
                    ),
                ),
            ),
        )
        audit = PostLinearRemainingFiniteSystemAudit(
            active_system_k_refinement(),
            missing_triangular_row_profile=forged_profile,
        )

        self.assertFalse(forged_section.rank_profile_row_exact)
        self.assertIn(
            "rank_does_not_match_kernel_blocks",
            forged_section.rank_profile_inexact_reasons,
        )
        self.assertFalse(forged_profile.rows[0].section_profile_ledger_exact)
        self.assertEqual(
            forged_profile.rows[0].explanation,
            "unclassified_missing_triangular_profile",
        )
        self.assertFalse(forged_profile.finite_map_classification_exhaustive)
        self.assertEqual(audit.system_name, "system_k_kink_completion_deficit")
        self.assertTrue(audit.system_k_active)
        self.assertIn(
            (
    ...
    def test_monodromy_front_door_builds_full_domain_constant_observers(self):
        ...
        bad_label_audit = (
            universal_k_endpoint_observer_builds_from_monodromy_by_family(
                ...
                derive_fibre_label_identity_residual_faithfulness=True,
                derive_canonical_fibre_label_identity_residual_faithfulness=True,
                fibre_label_identity_rows=(("*", 0, "only_one_label"),),
            )
        )

        self.assertFalse(bad_label_audit.proves_family_endpoint_observers)
        self.assertIn(
            "endpoint_observer_family_builds_not_proved",
            bad_label_audit.failure_reasons,
        )
        self.assertEqual(
            bad_label_audit.product_residual_theorem_channel_reasons,
            (),
        )
        self.assertEqual(
            bad_label_audit.product_residual_faithfulness_theorem.expected_residual_input_tuples,
            (
                (
    ...
    def test_coordinate_identity_residual_faithfulness_closes_coordinate_candidates(self):
    ...
    def test_canonical_fibre_label_identity_residual_faithfulness_derives_rows(self):
        ...
        canonical_rows = universal_k_canonical_fibre_label_identity_rows(interval)
        canonical_audit = universal_k_canonical_fibre_label_identity_audit(interval)

        self.assertEqual(
            tuple((color, point) for color, point, _label in canonical_rows),
            canonical_audit.expected_fibre_label_keys,
        )
        self.assertTrue(
            universal_k_interval_has_canonical_fibre_label_identity_action(interval)
        )
    ...
    def test_canonical_fibre_label_identity_refuses_noninjective_components(self):
        ...
        self.assertFalse(canonical_audit.proves_fibre_label_identity_action)
        self.assertIn(
            "fibre_label_identity_not_injective",
            canonical_audit.failure_reasons,
        )
        self.assertFalse(
            universal_k_interval_has_canonical_fibre_label_identity_action(interval)
        )
    ...
    def test_endpoint_observer_builds_by_family_cover_exact_ucm_families(self):
        ...
        self.assertTrue(
            family_audit_with_product
            .product_residual_faithfulness_channel_reasons_match_families
        )
        self.assertTrue(
            family_audit_with_product
            .product_residual_faithfulness_channel_keys_match_families
        )
        self.assertTrue(
        ...
        fake_local_channel_product_residual = replace(
            product_residual,
            residual_rows=tuple(
                replace(
                    row,
        ...
        fake_local_channel_family_audit = (
            universal_k_endpoint_observer_builds_by_family(
                interval,
                seed_entries,
                tuple(certificates),
                ...
                residual_faithfulness_theorems_by_family=tuple(residual_theorems),
                product_residual_faithfulness_theorem=(
                    fake_local_channel_product_residual
                ),
            )
        )

        self.assertTrue(fake_local_channel_product_residual.proves_residual_faithfulness)
        self.assertTrue(
            fake_local_channel_family_audit
            .product_residual_faithfulness_channel_reasons_match_families
        )
        ...
        self.assertFalse(
            fake_local_channel_family_audit.product_residual_faithfulness_proved
        )
        self.assertFalse(
            fake_local_channel_family_audit.proves_family_endpoint_product_closure
        )
        self.assertEqual(
            fake_local_channel_family_audit.product_residual_theorem_channel_keys_by_family,
            tuple(
                sorted(
                    (
                        (
        ...
        self.assertIn(
            "endpoint_observer_product_residual_faithfulness_channel_key_scope_mismatch",
            fake_local_channel_family_audit
            .product_residual_faithfulness_failure_reasons,
        )
    ...
    def test_monodromy_family_input_audit_rejects_stale_endpoint_emissions(self):
        interval = one_color_identity_interval()
        group = cyclic_group(2)
        seed_state = ("*", "*", "left_constant_map_universal_kernel")
        seed_key = ("U", seed_state)
        ...
        family_audit = universal_k_endpoint_observer_builds_from_monodromy_by_family(
            ...
            endpoint_groups_by_family=(("U", group),),
            word_potential_templates_by_family=(("U", ((seed_key, ()),)),),
            positive_state_rows_by_family=(("U", (stale_emission_row,)),),
        )

        self.assertFalse(family_audit.proves_family_endpoint_observers)
        self.assertEqual(
            family_audit.monodromy_family_input_audit.malformed_positive_state_rows,
            (("U", (stale_emission_row,)),),
        )
        self.assertIn(
            "endpoint_observer_monodromy_positive_rows_malformed_rows",
            family_audit.failure_reasons,
    ...
    def test_routed_u_identity_observer_closes_non_strict_residual_systems(self):
        refinement = constant_map_kernel_only_system_k_refinement()
        closure = TriangularLatinDefectClosureAudit(
            rows=(
                TriangularLatinDefectClosureRow(
        ...
        for case_name, interval, derive_kwargs, channel_reason in (
            residual_trivial_identity_observer_cases()
        ):
            with self.subTest(case=case_name):
                ...
                self.assertEqual(family_build.product_residual_theorem_channel_reasons, ())
                self.assertEqual(family_build.failure_reasons, ())
                self.assertTrue(family_build.proves_family_endpoint_product_closure)
                self.assertTrue(audit.system_u_closed_by_endpoint_observer_family_build)
                self.assertTrue(audit.all_active_routed_endpoint_systems_closed)
                ...
                self.assertEqual(audit.remaining_obligations, ())
                self.assertTrue(
                    data["endpoint_observer_family_build_closes_current_kappa"]
                )
                self.assertTrue(
    ...
    def test_routed_c_strict_identity_endpoint_observer_closes_system(self):
        interval = one_color_identity_interval()
        profile, closure, route = partial_constant_missing_row_profile_route_audits()
        refinement = OneNoTriangularRawKRefinement()
        routed = PostLinearRemainingFiniteSystemAudit(
            refinement,
            missing_triangular_row_profile=profile,
            missing_triangular_partial_constant_closure=closure,
        ...
        self.assertEqual(routed.active_routed_endpoint_systems, ("C",))
        self.assertEqual(family_build.expected_endpoint_families_exact, ("C",))
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
        ...
        self.assertTrue(data["endpoint_observer_family_build_closes_current_kappa"])
        self.assertTrue(data["endpoint_observer_family_build_rows_match_current_interval"])

    def test_routed_c_identity_observer_closes_non_strict_residual_systems(self):
        profile, closure, route = partial_constant_missing_row_profile_route_audits()
        refinement = OneNoTriangularRawKRefinement()
        routed = PostLinearRemainingFiniteSystemAudit(
            refinement,
    ...
    def test_routed_m_strict_identity_endpoint_observer_closes_system(self):
        interval = one_color_identity_interval()
        profile, coordinate_routing = coordinate_unit_mixed_context_route_audits()
        refinement = OneNoTriangularRawKRefinement()
        routed = PostLinearRemainingFiniteSystemAudit(
            refinement,
            missing_triangular_row_profile=profile,
        ...
        self.assertEqual(routed.active_routed_endpoint_systems, ("M",))
        self.assertEqual(family_build.expected_endpoint_families_exact, ("M",))
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
    ...
    def test_routed_m_identity_observer_closes_non_strict_residual_systems(self):
        profile, coordinate_routing = coordinate_unit_mixed_context_route_audits()
        refinement = OneNoTriangularRawKRefinement()
        routed = PostLinearRemainingFiniteSystemAudit(
            refinement,
    ...
    def test_routed_uc_strict_identity_endpoint_observer_closes_product_system(self):
        ...
        self.assertEqual(routed.active_routed_endpoint_systems, ("U", "C"))
        self.assertEqual(family_build.expected_endpoint_families_exact, ("C", "U"))
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
        self.assertTrue(audit.system_u_closed_by_endpoint_observer_family_build)
        self.assertTrue(audit.system_c_closed_by_endpoint_observer_family_build)
        self.assertTrue(audit.all_active_routed_endpoint_systems_closed)
        self.assertEqual(audit.system_name, "closed_by_endpoint_observer_family_build")
        self.assertEqual(audit.unclosed_routed_endpoint_systems, ())
        self.assertEqual(audit.remaining_obligations, ())
        self.assertEqual(
            data["endpoint_observer_family_build_closed_families"],
    ...
    def test_routed_uc_identity_observer_closes_non_strict_residual_product_systems(self):
        refinement = constant_map_kernel_system_k_refinement()
        profile, partial_closure, partial_route = (
            right_partial_constant_missing_row_profile_route_audits()
        )
    ...
    def test_routed_um_strict_identity_endpoint_observer_closes_product_system(self):
        interval = one_color_identity_interval()
        refinement = RecoveryAndMixedRawKRefinement()
        profile, coordinate_routing = coordinate_unit_mixed_context_route_audits()
        closure = TriangularLatinDefectClosureAudit(
            rows=(
                TriangularLatinDefectClosureRow(
        ...
        self.assertEqual(routed.active_routed_endpoint_systems, ("U", "M"))
        self.assertEqual(family_build.expected_endpoint_families_exact, ("M", "U"))
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
        self.assertTrue(audit.system_u_closed_by_endpoint_observer_family_build)
        self.assertTrue(audit.system_m_closed_by_endpoint_observer_family_build)
        ...
        self.assertTrue(data["endpoint_observer_family_build_closes_current_kappa"])
        self.assertTrue(data["endpoint_observer_family_build_rows_match_current_interval"])
        self.assertTrue(data["endpoint_observer_family_build_uses_current_u_tri_target"])

    def test_routed_um_identity_observer_closes_non_strict_residual_product_systems(self):
        refinement = RecoveryAndMixedRawKRefinement()
        profile, coordinate_routing = coordinate_unit_mixed_context_route_audits()
        closure = TriangularLatinDefectClosureAudit(
            rows=(
    ...
    def test_routed_ucm_strict_identity_endpoint_observer_closes_product_system(self):
        interval = one_color_identity_interval()
        refinement = RecoveryContinuationMixedRawKRefinement()
        profile, partial_closure, partial_route, coordinate_routing = (
            continuation_and_mixed_context_route_audits()
        )
        closure = TriangularLatinDefectClosureAudit(
            rows=(
        ...
        self.assertEqual(routed.active_routed_endpoint_systems, ("U", "C", "M"))
        self.assertEqual(
            family_build.expected_endpoint_families_exact,
            ("C", "M", "U"),
        )
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
    ...
    def test_routed_ucm_identity_observer_closes_non_strict_residual_product_systems(self):
        refinement = RecoveryContinuationMixedRawKRefinement()
        profile, partial_closure, partial_route, coordinate_routing = (
            continuation_and_mixed_context_route_audits()
        )
        ...
        for case_name, interval, derive_kwargs, channel_reason in cases:
            with self.subTest(case=case_name):
                ...
                self.assertEqual(
                    routed.active_routed_endpoint_systems,
                    ("U", "C", "M"),
                )
                self.assertEqual(
                    family_build.product_residual_theorem_channel_reasons,
    ...
    def test_routed_cm_strict_identity_endpoint_observer_closes_product_system(self):
        interval = one_color_identity_interval()
        profile, closure, route, coordinate_routing = (
            continuation_and_mixed_context_route_audits()
        )
        ...
        self.assertEqual(family_build.expected_endpoint_families_exact, ("C", "M"))
        self.assertEqual(family_build.failure_reasons, ())
        self.assertTrue(family_build.proves_family_endpoint_product_closure)
        self.assertTrue(audit.system_c_closed_by_endpoint_observer_family_build)
        self.assertTrue(audit.system_m_closed_by_endpoint_observer_family_build)
