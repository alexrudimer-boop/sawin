...
class UniversalKEndpointTargetAudit:
    ...
    def duplicate_target_endpoint_families(self) -> Tuple[str, ...]:
        families = tuple(
            family
            for family, _target_size in self.endpoint_group_orders + self.cutoff_degrees
        )
...
class UniversalKTelescopingDetectorAudit:
    ...
    and tied to a potential whose terminal value is generated by the final
    evaluated recursive longitudes.
    """
    ...
    def detector_tracks_supplied(self) -> bool:
        return self.detector_track_count is not None and self.detector_track_count > 0
    ...
    def proves_telescoping_detector_lift(self) -> bool:
        return (
            ...
            and self.telescoping_identity_verified
    ...
    def failure_reasons(self) -> Tuple[str, ...]:
        ...
        if not self.telescoping_identity_verified:
            reasons.append("endpoint_potential_telescoping_not_verified")
        ...
        if not self.initial_readout_normalized:
            reasons.append("endpoint_potential_initial_value_not_normalized")
        if not self.braid_index_independent:
    ...
    def _universal_k_signed_endpoint_generator_data(
        ...
        return (
            ...
            (
                "signed_endpoint_generator_endpoint_potential_telescoping_verified",
                (
                    telescoping_audit.telescoping_identity_verified
                    if telescoping_audit is not None
                    else False
                ),
            ),
