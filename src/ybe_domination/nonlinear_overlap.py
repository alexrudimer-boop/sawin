...
class UniversalKResidualFaithfulnessAudit:
    ...
    @property
    def residual_endpoint_channel_keys_by_family(
        self,
    ) -> Tuple[Tuple[str, Tuple[Tuple[object, ...], ...]], ...]:
        """Full endpoint-channel keys used by each routed endpoint family."""

        keys_by_family: dict[str, list[Tuple[object, ...]]] = {}
        for row in self.residual_rows:
            for key in row.endpoint_channel_keys:
                if not _universal_k_residual_endpoint_channel_key_well_formed(key):
                    continue
                family = key[0]
                keys_by_family.setdefault(family, []).append(key)
        return tuple(
            sorted(
                (
                    (family, tuple(sorted(_unique_values(tuple(keys)), key=repr)))
...
def universal_k_interval_has_strict_identity_fibre_action(
    interval: LocalInterval,
) -> bool:
...
def universal_k_canonical_strand_carrier_rows(
    interval: LocalInterval,
) -> Tuple[UniversalKStrandCarrierRow, ...]:
    """Derive the least carrier ledger forced by Artin strand preservation."""
        ...
        for input_left in interval.fibres[left_color]:
            for input_right in interval.fibres[right_color]:
                ...
                if output is None:
                    continue
                output_left, output_right = output
                union((right_color, input_right), (output_left_color, output_left))
                union((left_color, input_left), (output_right_color, output_right))

    roots = tuple(sorted({find(node) for node in parent}, key=repr))
    carrier_by_root = {
        root: ("canonical_strand_carrier_component", index)
...
def universal_k_canonical_strand_carrier_soundness_audit(
    interval: LocalInterval,
) -> UniversalKStrandCarrierSoundnessAudit:
    """Audit the canonical least strand-carrier preservation ledger."""

    return universal_k_strand_carrier_soundness_audit(
        interval,
        universal_k_canonical_strand_carrier_rows(interval),
    )


def universal_k_interval_has_canonical_strand_carrier_action(
    interval: LocalInterval,
) -> bool:
    """Return whether canonical strand carriers are fibrewise injective."""

    return _universal_k_interval_table_complete_and_type_correct(
        interval
    ) and universal_k_canonical_strand_carrier_soundness_audit(
        interval
    ).proves_injective_strand_carrier_soundness
...
def _universal_k_trivial_residual_faithfulness_audit(
    ...
    *,
    ...
    if theorem_holds:
        ...
        for family in active_families:
            ...
            row_list.append(
                UniversalKResidualFaithfulnessRow(
                    ...
                    identity_endpoint_output_tuple=input_tuple,
        ...
        rows = tuple(row_list)
    family_row_counts = tuple((family, 1) for family in active_families)
    return UniversalKResidualFaithfulnessAudit(
        active_endpoint_families=active_families,
        covered_endpoint_families=active_families,
...
def universal_k_coordinate_identity_residual_faithfulness_audit(
    interval: LocalInterval,
    endpoint_seed_states: Sequence[Tuple[str, UniversalKSeedState]],
) -> UniversalKResidualFaithfulnessAudit:
    """Prove residual faithfulness when local rows preserve fibre coordinates."""
...
def universal_k_canonical_strand_carrier_residual_faithfulness_audit(
    interval: LocalInterval,
    endpoint_seed_states: Sequence[Tuple[str, UniversalKSeedState]],
) -> UniversalKResidualFaithfulnessAudit:
    """Prove residual faithfulness from the canonical strand-carrier quotient."""

    return _universal_k_trivial_residual_faithfulness_audit(
        endpoint_seed_states,
        row_reason="canonical_strand_carrier_residual_channel",
        theorem_holds=universal_k_interval_has_canonical_strand_carrier_action(
            interval
        ),
    )
...
def universal_k_automatic_residual_faithfulness_audit(
    interval: LocalInterval,
    endpoint_seed_states: Sequence[Tuple[str, UniversalKSeedState]],
    *,
    strand_carrier_rows: object = (),
    ...
    present, supplied fibre-label identity when a ledger is present, canonical
    strand-carrier identity, and canonical fibre-label identity.  If a
    supplied carrier or label ledger is present but fails, that failed audit is
    returned so stale or malformed certificate data is not hidden by the
    canonical fallback.
    """
    ...
    if canonical_strand.proves_residual_faithfulness:
        return canonical_strand
...
class UniversalKEndpointObserverFamilyBuildAudit:
    ...
    @property
    def residual_theorem_channel_keys_by_family(
        self,
    ) -> Tuple[Tuple[str, Tuple[Tuple[object, ...], ...]], ...]:
        return tuple(
            sorted(
                (
                    (family, keys)
                    for _row_family, theorem in self.residual_theorem_row_parts
                    for family, keys in theorem.residual_endpoint_channel_keys_by_family
                    if _is_hashable(family)
                    and family in UNIVERSAL_K_ENDPOINT_FAMILIES
                ),
                key=repr,
    ...
    @property
    def product_residual_theorem_channel_keys_by_family(
        self,
    ) -> Tuple[Tuple[str, Tuple[Tuple[object, ...], ...]], ...]:
        if self.product_residual_faithfulness_theorem is None:
            return ()
        return (
            self.product_residual_faithfulness_theorem
            .residual_endpoint_channel_keys_by_family
        )
    ...
    @property
    def monodromy_input_seed_classifier_mismatches(
        self,
    ) -> Tuple[Tuple[str, object], ...]:
        audit = self.monodromy_family_input_audit
        if audit is None:
            return ()
        current_entries = tuple(self.seed_classifier_entries)
        supplied_entries = tuple(audit.seed_classifier_entries)
        current_markers = _value_marker_set(current_entries)
        supplied_markers = _value_marker_set(supplied_entries)
        supplied_parts = tuple(
            parts
        ...
        targets_by_descriptor: dict[object, set[object]] = {}
        descriptor_by_marker: dict[object, object] = {}
        for descriptor, target in supplied_parts:
            descriptor_marker = _value_marker(descriptor)
            descriptor_by_marker.setdefault(descriptor_marker, descriptor)
            targets_by_descriptor.setdefault(descriptor_marker, set()).add(
                _value_marker(target)
        ...
        missing_current = tuple(
            ...
            if _value_marker(entry) not in supplied_markers
        )
        if missing_current:
            failures.append(
                (
        ...
        extra_stale = tuple(
            entry
            for entry in supplied_entries
            if _value_marker(entry) not in current_markers
        )
        if extra_stale:
            failures.append(
                ("monodromy_seed_classifier_extra_stale_entries", extra_stale)
            )
        duplicate_entries = _duplicate_values(supplied_entries)
        if duplicate_entries:
            failures.append(
                ("monodromy_seed_classifier_duplicate_entries", duplicate_entries)
            )
        duplicate_descriptors = _duplicate_values(
            tuple(descriptor for descriptor, _target in supplied_parts)
        )
        if duplicate_descriptors:
            failures.append(
                (
                    "monodromy_seed_classifier_duplicate_descriptors",
                    duplicate_descriptors,
                )
            )
        conflicting_descriptors = tuple(
            sorted(
                (
                    descriptor_by_marker[descriptor_marker]
                    for descriptor_marker, targets in targets_by_descriptor.items()
        ...
        if conflicting_descriptors:
            failures.append(
                (
                    "monodromy_seed_classifier_conflicting_descriptors",
    ...
    @property
    def monodromy_input_seed_classifier_matches_current(self) -> bool:
        return not self.monodromy_input_seed_classifier_mismatches
    ...
    def monodromy_input_rows_match_builds(self) -> bool:
        return (
            self.monodromy_input_seed_classifier_matches_current
            and not self.monodromy_input_build_mismatches
        )
    ...
    @property
    def product_residual_faithfulness_channel_keys_match_families(self) -> bool:
        if self.product_residual_faithfulness_theorem is None:
            return False
        return (
            self.product_residual_theorem_channel_keys_by_family
            == self.residual_theorem_channel_keys_by_family
        )
    ...
    def product_residual_faithfulness_proved(self) -> bool:
        ...
        return (
            ...
            and self.product_residual_faithfulness_channel_keys_match_families
    ...
    def product_residual_faithfulness_failure_reasons(self) -> Tuple[str, ...]:
        ...
        if not self.product_residual_faithfulness_channel_keys_match_families:
            reasons.append(
    ...
    def failure_reasons(self) -> Tuple[str, ...]:
        ...
        if self.duplicate_seed_classifier_entries:
            reasons.append("endpoint_observer_family_seed_classifier_duplicate_entries")
        if self.duplicate_seed_classifier_descriptors:
            reasons.append(
        ...
        if self.monodromy_input_seed_classifier_mismatches:
            reasons.append(
...
def _universal_k_residual_rows_with_identity_helpers(
    ...
    *,
    ...
    derive_canonical_strand_carrier_residual_faithfulness: bool = False,
    ...
    def derive_for_seed_states(
        ...
        if (
            derive_canonical_strand_carrier_residual_faithfulness
    ...
    helpers_requested = (
        ...
        or derive_canonical_strand_carrier_residual_faithfulness
...
def universal_k_identity_endpoint_observer_builds_by_family(
    ...
    *,
    ...
    residual_rows, product_residual_faithfulness_theorem = (
        _universal_k_residual_rows_with_identity_helpers(
            interval,
            seed_classifier_entries,
            residual_faithfulness_theorems_by_family,
...
class PostLinearRemainingFiniteSystemAudit:
    ...
    def _universal_k_endpoint_observer_family_build_data(
        ...
        return (
            ...
            (
                "endpoint_observer_family_build_product_residual_faithfulness_channel_key_scope_matches",
                audit.product_residual_faithfulness_channel_keys_match_families,
            ),
            (
                "endpoint_observer_family_build_product_residual_faithfulness_failure_reasons",
            ...
            (
                "endpoint_observer_family_build_product_residual_faithfulness_channel_keys_by_family",
                audit.product_residual_theorem_channel_keys_by_family,
            ),
            (
            ...
            (
                ...
                audit.residual_theorem_channel_keys_by_family,
            ),
            (
                "endpoint_observer_family_residual_theorem_malformed_rows",
            ...
            (
                "endpoint_observer_monodromy_input_seed_classifier_matches_current",
                audit.monodromy_input_seed_classifier_matches_current,
            ),
            (
                "endpoint_observer_monodromy_input_seed_classifier_mismatches",
                audit.monodromy_input_seed_classifier_mismatches,
            ),
            (
            ...
            (
                "endpoint_observer_monodromy_detector_domain_malformed_rows",
                audit.monodromy_family_input_audit.malformed_detector_domain_assignment_rows
                if audit.monodromy_family_input_audit is not None
                else (),
            ),
            (
            ...
            (
                "endpoint_observer_monodromy_detector_witness_malformed_rows",
                audit.monodromy_family_input_audit.malformed_detector_domain_witness_rows
                if audit.monodromy_family_input_audit is not None
                else (),
            ),
            (
...
def post_linear_remaining_finite_system_audit(
    ...
    *,
    ...
    universal_k_monodromy_word_potential_templates_by_family: object = (),
    ...
    universal_k_identity_canonical_strand_carrier_residual_faithfulness: bool = False,
    ...
    derive_identity_endpoint_observer_family_build = (
        ...
        or universal_k_identity_canonical_strand_carrier_residual_faithfulness
    ...
    monodromy_family_input_present = (
        bool(
            _universal_k_row_input_tuple(
    ...
    derive_endpoint_observer_family_build = (
        ...
        and (
            monodromy_family_input_present
            or direct_family_input_present
