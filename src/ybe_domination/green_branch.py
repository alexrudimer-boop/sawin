...
@dataclass(frozen=True)
class GreenSectionTransportRigidityCase:
    """One row in the actual completed-context section-rigidity gate."""
...
@dataclass(frozen=True)
class GreenSectionTransportRigidityGateAudit:
    """Formal gate for Green section-holonomy rigidity.

    This is not a finite-depth computation.  It records the exact symbolic
    lemma left after the Green atom layer and bounded group-loop guardrail:
    comparable section branches must be compared only by actual deterministic
    completed-context morphisms generated by supported YBE rows.
    """

    actual_supported_transport_required: bool
    raw_rees_normalization_rejected: bool
    recurrent_hidden_component_required: bool
    cyclic_p_primary_fibre_required: bool
    ...
    @property
    def records_green_section_transport_rigidity_gate(self) -> bool:
        return (
            self.actual_supported_transport_required
            and self.raw_rees_normalization_rejected
            and self.recurrent_hidden_component_required
            and self.cyclic_p_primary_fibre_required
...
def green_section_transport_rigidity_gate_audit() -> GreenSectionTransportRigidityGateAudit:
    """Record the exact Green section-holonomy rigidity decision point.

    The completed-context observer already separates reset-like atom-trivial
    collapse from group-like loops.  The remaining symbolic issue is whether
    two comparable hidden section branches can induce different units on a
    cyclic p-primary branch fibre after actual supported transport.
    """

    cases = (
        GreenSectionTransportRigidityCase(
            key="actual_completed_context_transport",
            role="model_requirement",
            statement=(
                "section branches e and e' are comparable only when there is "
                "a deterministic completed-context morphism theta_{e,e'} "
                "generated by supported YBE rows between their surviving "
                "hidden subsystems"
            ),
            consequence=(
                "raw equality of Green or Schutzenberger coordinates is not "
                "evidence of comparability unless it is realized by actual "
                "context transport"
            ),
        ),
        GreenSectionTransportRigidityCase(
        ...
        GreenSectionTransportRigidityCase(
            ...
            role="guardrail",
            statement=(
                "Rees coordinates, Green R-class normalization, or the full "
                "Schutzenberger action may move formal labels outside the "
                "supported completed-row category"
            ),
            consequence=(
                "a proof or counterexample must use objectwise supported rows "
                "and YBE cube transports, not ambient semigroup bookkeeping"
            ),
        ),
        GreenSectionTransportRigidityCase(
            key="recurrent_hidden_component_filter",
            role="hypothesis",
            statement=(
                "the branch component Omega must be recurrent, alpha-trivial, "
                "same-atom, and depth-null after the finite atom and "
                "Green/Schutzenberger readouts"
            ),
            consequence=(
                "reset artifacts and already visible atom or Schutzenberger "
                "motion are excluded before testing section holonomy"
            ),
        ),
        GreenSectionTransportRigidityCase(
            key="cyclic_p_primary_section_fibre",
            role="test_object",
            statement=(
                "the residual section fibre is tested on a cyclic p-primary "
                "quotient N_{C,d}/N_{C,d+1}, so possible branch holonomy lands "
                "in Aut(C_{p^m})"
            ),
            consequence=(
                "the first sharp obstruction is a unit mismatch on this cyclic "
                "quotient, not a nonunit or nonbijective context collapse"
            ),
        ),
        GreenSectionTransportRigidityCase(
        ...
        GreenSectionTransportRigidityCase(
            key="negative_countercertificate_target",
            role="negative_route",
            statement=(
                "a counterexample must give a finite bijective YBE solution, a "
                "full regular R-class, actual comparable section branches, "
                "and unequal induced units in Aut(C_{p^m})"
            ),
            consequence=(
        ...
        GreenSectionTransportRigidityCase(
            key="finite_depth_group_loop_guardrail",
            role="finite_prefix_guardrail",
            statement=(
                "bounded_atom_trivial_loop_group_summaries detects only total "
                "bijective atom-trivial loops and ignores reset-like collapses"
            ),
            consequence=(
                "finite-depth searches can refute candidate examples that "
                "have no group-like loop, but the all-depth rigidity lemma is "
                "still symbolic"
            ),
        ),
    )
    return GreenSectionTransportRigidityGateAudit(
        actual_supported_transport_required=True,
        raw_rees_normalization_rejected=True,
        recurrent_hidden_component_required=True,
        cyclic_p_primary_fibre_required=True,
