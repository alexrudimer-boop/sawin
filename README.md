...
apply.  The abelian layer is now an exact matrix problem:
`proofs/abelian_longitude_image_criterion.md` proves that for finite abelian
`A`, `V_beta(A)` is generated by `a^{m_ij}` over the entries of the
...
`proofs/continuation_readout_propagation.md` removes another layer of repeated
work: if a fixed readout relation is an admissible local congruence and
contains one representative continuation seed, then it contains the whole
least admissible closure generated by that seed.  The helper
`continuation_seed_readout_propagation_audits(...)` records admissibility, seed
containment, and any missing generated edges.
The companion readout-kernel criterion
`proofs/readout_kernel_admissibility.md` makes the admissibility input
...
The inverse formula is now isolated in
`proofs/triangular_bundle_recovery_inverse.md`: an output `(u,v)` recovers
the unique block label `x=r_u(v)` and then the unique within-block coordinate
`y=beta_x^{-1}(v)`.  The helper `triangular_recovery_audit(...)` records this
output-to-source recovery table.  The remaining triangular task is therefore
recovery holonomy: longitudinalize the finite block-label and within-block
inverse transports, or upgrade one recovery cycle to normalized laws.
The constant-column collapse note
`proofs/constant_column_collapse_triangular.md` removes the non-Latin half of
that triangular residue.  If a hidden opposite column
`C_y(x)=beta_x(y)` is non-bijective, then Green/Schutzenberger visibility
rules out a proper kernel, the column is constant, and bijectivity of the
companion sections forces every opposite column to be constant.  The row
collapses to product/permutation form.  The helper
`triangular_column_collapse_audit(...)` records product-collapse rows and the
remaining Latin-unit triangular rows.  The final triangular target is now the
Latin-unit triangular longitude theorem for the fixed group generated by the
triangular unit transports.
The Latin triangular YBE split
`proofs/latin_triangular_ybe_split.md` separates that target again: the first
coordinate equation forces the bijections `alpha_{a,b}` to satisfy a finite
product/permutation cocycle
`alpha_{a.b,(a*b).c} alpha_{a,b}=alpha_{a,b.c}`.  Thus alpha transport routes
to the closed product/permutation branch.  The helper
`latin_triangular_ybe_audit(...)` records this alpha-cocycle equation together
